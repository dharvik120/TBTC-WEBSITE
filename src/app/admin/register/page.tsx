...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await registerAdmin({ username, email, passwordPass: password });
      if (res.error) {
        setError(res.error);
        setLoading(false);
      } else {
      ...
      <div className="max-w-md w-full bg-slate-950 border border-slate-800 rounded-md p-6 lg:p-8 shadow-2xl relative">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <p className="text-slate-500 text-xs mt-1.5 leading-relaxed">
            Please register the primary administrator login credentials to configure and manage the platform.
          </p>
        </div>
